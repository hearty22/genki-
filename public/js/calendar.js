(function(){
    ...
    function render() {
        ...
        for (let i = 0; i < totalCells; i++) {
            ...
            if (formatISO(today) === iso) {
                cell.classList.add('today');
                todayShort.textContent = today.toLocaleDateString('es-ES', {
                    day: 'numeric',
                    month: 'short'
                });
            }
            if (selectedDate && iso === selectedDate) {
            ...
            if (events[iso]) {
                ...
                events[iso].forEach(e => {
                    if (activeFilters.includes(e.type)) {
    ...
    function updateSelectedPanel() {
        if (selectedDate) {
            ...
            selectedFull.textContent = pretty(d);
            selectedShort.textContent = selectedDate;
        } else {
            selectedFull.textContent = 'Selecciona una fecha';
    ...
    function renderEventsList() {
        ...
        if (selectedDate) {
            ...
            if (dayEvents.length === 0) {
                ...
                p.className = 'small';
                p.textContent = 'No hay eventos visibles.';
            ...
            dayEvents.forEach(item => {
                ...
                btn.addEventListener('click', () => removeEvent(selectedDate, item.id));
                eventsList.appendChild(el);
            });
        }
    }

    function addEvent(date, text, type) {
        if (!text) return;
    ...
    function getActiveFilters() {
        return Array.from(filterInputs).filter(f => f.checked).map(f => f.value);
    }
    ...
    addEventBtn.addEventListener('click', () => {
        if (!selectedDate) selectedDate = formatISO(new Date());
        updateSelectedPanel();
        const text = eventText.value.trim();
        if (!text) return eventText.focus();
        addEvent(selectedDate, text, eventType.value);
    });
